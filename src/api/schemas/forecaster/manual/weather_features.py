...
def add_weather_features(df: pd.DataFrame, given_data: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df = df.sort_values("time").reset_index(drop=True)
    df["temperature_2m"] = given_data["temperature_2m"].values[0]
    df["relative_humidity_2m"] = given_data["relative_humidity_2m"].values[0]
    df["wind_speed_10m"] = given_data["wind_speed_10m"].values[0]
    df["wind_direction_10m"] = given_data["wind_direction_10m"].values[0]
    df["surface_pressure"] = given_data["surface_pressure"].values[0]
    df["precipitation"] = given_data["precipitation"].values[0]
    def get_weather(row):
