...
def add_ratio_features(df: pd.DataFrame, given_data: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["time"] = pd.to_datetime(df["time"])
    df = df.sort_values("time").reset_index(drop=True)
    df["pm10_lag_12h"] = given_data["pm_10_lag_12h"].values[0]
    df["pm10_lag_24h"] = given_data["pm_10_lag_24h"].values[0]
    df["pm10_lag_48h"] = given_data["pm_10_lag_48h"].values[0]
    ...
    df["pm_ratio_lag_12h"] = df["pm10_lag_12h"] / df["pm2_5_lag_12h"]
    df["pm_ratio_lag_24h"] = df["pm10_lag_24h"] / df["pm2_5_lag_24h"]
    df["pm_ratio_lag_48h"] = df["pm10_lag_48h"] / df["pm2_5_lag_48h"]
    ...
    past_pm_ratio = df["pm_ratio"].shift(1)

    df["pm_ratio_rolling_mean_24h"] = (past_pm_ratio.rolling(window=4).mean())

    df["pm_ratio_rolling_mean_48h"] = (past_pm_ratio.rolling(window=8).mean())
