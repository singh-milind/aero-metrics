...
def add_weather_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["dry_stagnation"] = df["relative_humidity_2m"] / (df["wind_speed_10m"] + 1)
    df["no_rain"] = (df["precipitation"] == 0).astype(int)
    df["weather_verdict"] = df.apply(get_weather, axis=1)

    df = df.sort_values("time").reset_index(drop=True)
    weather_cols = ["temperature_2m", "relative_humidity_2m", "wind_speed_10m", "surface_pressure", "precipitation"]
