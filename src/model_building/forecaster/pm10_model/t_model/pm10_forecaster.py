...
from src.model_building.forecaster.pm10_model.t_model.runtime_features import build_more_features
from src.model_building.forecaster.pm10_model.t_model.split_data import split_data
from src.model_building.forecaster.pm10_model.t_model.train_model import train_model
from src.utils.blob_storage import load_csv
from src.utils.blob_storage import upload_model


ROOT_DIR = Path(__file__).resolve().parents[5]
logger = get_logger("pm10_forecaster_t")
...
def upload_model_to_blob(model,explainer,global_shap_importance,global_shap_values,X_test, logger):
    ...
    upload_model(
        ...
        blob_name="forecaster/pm10/t_model/pm10_forecaster.pkl",
        model=model,
    ...
    upload_model(
        container_name="models",
        blob_name="forecaster/pm10/t_model/pm10_explainer.pkl",
        model=explainer,
    ...
    upload_model(
        container_name="models",
        blob_name="forecaster/pm10/t_model/pm10_global_shap_importance.pkl",
    ...
    upload_model(
        container_name="models",
        blob_name="forecaster/pm10/t_model/pm10_global_shap_values.pkl",
    ...
    upload_model(
        container_name="models",
        blob_name="forecaster/pm10/t_model/pm10_global_shap_feature_values.pkl",
        model=X_test,
        logger=logger,
...
def main():
    ...
    model_dir = ROOT_DIR / "models" / "forecaster" / "pm10" / "t_model"
    model_dir.mkdir(parents=True, exist_ok=True)

    joblib.dump(model, model_dir / "pm10_forecaster_t.pkl")
