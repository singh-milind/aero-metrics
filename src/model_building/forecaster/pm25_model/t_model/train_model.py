...
def train_model(x_train, y_train, logger):
    ...
    hp = params["pm25_forecaster_t"]["hyperparameters"]
    ...
    mlflow.set_experiment("pm25_forecaster_t_production")
    ...
    upload_metric(
        ...
        blob_name="forecaster/pm25_model/t_model_metrics.json",
        metric=metrics,
        logger=logger,
        artifact_name="Forecaster PM 2.5 t Metrics"
    )
    dump_metrics_json(metrics, "forecaster", "pm25_model", "t_model")
    logger.info(
        "Metrics saved to metrics/forecaster/t_model_metrics.json"
    )
