...
from src.database.ingest_future_data import ingest
...
def main():
    ...
    logger.info("Starting weather data fetch for all cities...")

    india_weather_df = get_weather_data_for_all_cities()

    if not india_weather_df.empty:
        ingest(india_weather_df)
    ...
    logger.info(f"Weather data fetch completed in {elapsed_time:.2f} seconds.")
    ingest(india_weather_df)
    logger.info("Dataset saved to database successfully.")
